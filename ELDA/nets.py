...
class MFNet(nn.Module):
    ...
    参数:
        n_users (int):模型中的用户数量。
        n_items (int):模型中的项数。
        Embedding_dim: int，可选潜在表征的维度

        偏见(bool):
        User_embedding_layer:可选的嵌入层
            如果提供，将被用作用户嵌入层网络的。
        Item_embedding_layer:可选的嵌入层 如果提供，将用作项目嵌入层网络的。
        稀疏:布尔值，可选 使用稀疏梯度。
    """
    ...
    def forward(self, user_ids, item_ids):
        """
            计算表示的前向传递。
        参数:
            user_ids (tensor):用户索引张量。
            item_ids (tensor):项索引张量。
        返回:
            张量:预测张量。
        """
        ...
        if dot.dim() > 1:  # 处理embedding_dim=1的情况
            dot = dot.sum(1)
